...
plot_predicted_vs_actual <- function(input, title = NULL, col = rgb(0,0,0,0.1), col_1to1 = 'red', col_fit = 'blue', legend = TRUE){
  ...
  # if pval rounds to zero, display as <0.001
  if(pval == 0){
    pval <- '<0.001'
    main <- paste(title, '\n', 'R = ', cor_est, ' | p < 0.001', sep = '')
  } else {
    main <- paste(title, '\n', 'R = ', cor_est, ' | p = ', pval, sep = '')
  }
  
  maxVal <- max(c(input$actual, input$predicted), na.rm = T)
...
# combined the two functions used for overall/conditional models into one to pass the p-value directly to the plot

model_plot_R_mortality <- function(mod, re.form = NULL, se.fit = FALSE, # model parameters
                              title = NULL, # plot parameters
                              col = rgb(0,0,0,0.1),
                              col_fit = 'blue'){
  ...
  mort.info <- summary(mort)
  
  # combine outputs into list  
  out <- list(actual = df$alive, predicted = df$pred, cor = cor, data_noNAs = noNA)
  
  # plot
  pval <-  round(mort.info$coefficients[2,'Pr(>|z|)'], 3)
  # if pval rounds to zero, display as <0.001
  if(pval == 0){
    pval <- '<0.001'
    main <- paste(title, '\np < 0.001', sep = '')
  ...
  # save plot to output
  out$plot <- ggplot(data = df, aes(x = pred, y = alive,)) +
  geom_jitter(height = 0.05, width = 0, color = col) +
  xlab('Probability of Mortality') +
  ylab('Actual Survival')+
    ggtitle(main) +
geom_smooth(method = glm, formula = y ~ x, method.args = list(family = binomial), col = col_fit)
...
# which model to save? (year_indiv_RE)
mod <- glmmTMB(log(pheno + 1) ~ garden_clim*home_clim + garden_clim_2*home_clim_2  + garden_clim_2 * home_clim + home_clim_2* garden_clim +  pc1*garden_clim + pc2*garden_clim + pc3*garden_clim + pc1*garden_clim_2 + pc2*garden_clim_2 + pc3*garden_clim_2 +
                      (1 | genotype) + (1 | garden/block) +(1 | year) + (1 | indiv),
               data = df.scaled,
               family = gaussian(),
               ziformula = ~.)
...
# png(file = paste('results/model_prediction/glmTMB_full_model_actual_vs_predicted_', garden_clim_colname, '_vs_', pheno_colname, '1to1.png', sep = ''),
#     height = 10,
#     width = 10,
#     res = 300,
#     units = 'in')
...
# TODO surpress text output
# plot predicted vs actual for mortality
p1 <- model_plot_R_mortality(mod, re.form = NULL, title = 'With random effects')
plot(p1$plot)
#ggsave(filename = paste('results/model_prediction/glmTMB_full_model_zero-inflated_withREs_actual_vs_predicted_', garden_clim_colname, '_vs_', pheno_colname, '.png', sep = ''), height = 5, width = 6)
p2 <- model_plot_R_mortality(mod, re.form = NA, title = 'No random effects')
plot(p2$plot)
#ggsave(filename = paste('results/model_prediction/glmTMB_full_model_zero-inflated_noREs_actual_vs_predicted_', garden_clim_colname, '_vs_', pheno_colname, '.png', sep = ''), height = 5, width = 6)
...
# plot actual vs predicted survival
for(n in 1:length(mods_test)){
  
  modN <- mods_test[[n]]
  
  # plot predicted vs actual for mortality
  p1 <-  model_plot_R_mortality(modN, re.form = NULL, 
                                title = paste(names(mods_test)[n], '\n', 'With random effects', sep = ''))
  
  p2 <- model_plot_R_mortality(modN, re.form = NA, 
                               title = paste(names(mods_test)[n], '\n', 'Without random effects', sep = ''))
  
  plot(p1$plot + p2$plot)
...
# function for predicting mortality and plotting expected vs observed with new data
model_plot_R_mortality_new <- function(mod, newdata, re.form = NULL, se.fit = FALSE, # model parameters
                              title = NULL, # plot parameters
                              col = rgb(0,0,0,0.1), 
                              col_fit = 'blue'){
  ...
  # predict phenotype - here, probability of mortality
  noNA$pred <- predict(mod, newdata = noNA, type = 'zprob', re.form = re.form, se.fit = se.fit)
  
  # convert growth increment to binary 0/1
  # if value is not 0, convert to 1
  noNA$alive <- ifelse(noNA$pheno == 0, 0, 1)
  
  # logistic model
  mort <- glm(alive ~ pred, family = 'binomial', data = noNA)
  mort.info <- summary(mort)
  
  # combine outputs into list  
  out <- list(actual = noNA$alive, predicted = noNA$pred, cor = cor, data_noNAs = noNA)
  ...
  # save plot to output
  out$plot <- ggplot(data = noNA, aes(x = pred, y = alive,)) +
  geom_jitter(height = 0.05, width = 0, color = col) +
  xlab('Probability of Mortality') +
  ylab('Actual Survival')+
    ggtitle(main) +
...
for(n in 1:length(gards)){
  ...
  # zero-inflated model
  out1 <-  model_plot_R_mortality_new(modN, 
                                      newdata = newdata, 
                                      re.form = NULL, 
                                title = paste(names(mods_test)[n], '\n', 'With random effects', sep = ''))
  p1 <- out1$plot
  
  out2 <- model_plot_R_mortality_new(modN, 
                                     newdata = newdata, 
                                     re.form = NA, 
                               title = paste(names(mods_test)[n], '\n', 'With random effects', sep = ''))
  p2 <- out2$plot
...
names(pred.zi_REs) <- model_names
names(pred.zi_noREs) <- model_names

##########################
# plot actual vs expected heights for each garden in a panel

# combine overall and conditional into a list
# leave out zreo-inflated because we can't calculate R values for a glm with binomial link
...
gard_cols <- c("#008856",  "#8DB600", "#BE0032", "#875692", "#A1CAF1", "#2B3D26", "#C2B280", "#F3C300", "#848482", "#0067A5", "#E68FAC", "#F99379", "#604E97", "#DCD300",  "#B3446C", "#882D17", "#E25822", "#654522")


# png(filename = paste('results/model_prediction/validation_leaveOneGardenOutAndPredict_', garden_clim_colname, '_vs_', pheno_colname, '_1to1axes.png', sep = ''), height = 7.2, width = 12.8, units = 'in', res = 300)
# par(mfrow = c(3,6))

#pdf(file = paste('results/model_prediction/validation_leaveOneGardenOutAndPredict_', garden_clim_colname, '_vs_', pheno_colname, '_1to1axes.pdf', sep = ''), height = 8, width = 12)
#par(mfrow = c(3,4))
...
#pdf(file = paste('results/model_prediction/validation_leaveOneGardenOutAndPredict_sortGardensByMortality', garden_clim_colname, '_vs_', pheno_colname, '_1to1axes.pdf', sep = ''), height = 8, width = 12)
...
#png(filename = paste('results/model_prediction/validation_leaveOneGardenOutAndPredict_sortGardensByMortality', garden_clim_colname, '_vs_', pheno_colname, '_1to1axes.png', sep = ''), height = 7.2, width = 12.8, units = 'in', res = 300)
...
# because of the way I set up the functions, have to re-run the models here to change plot parameters
mort_plots <- list()
for(n in 1:length(gards)){
  ...
  # zero-inflated model
  out <-  model_plot_R_mortality_new(modN, 
                                     newdata = newdata, 
                                     re.form = NULL, 
                                     title = gard,
                                     col = gard_cols[n],
                                     col_fit = gard_cols[n])
  p <- out$plot + theme(text=element_text(size=10))
  #plot(p)
...
wrap_plots(mort_plots, ncol = 6)

#ggsave(filename = paste('results/model_prediction/validation_leaveOneGardenOutAndPredict_zeroInflated', garden_clim_colname, '_vs_', pheno_colname, '_1to1axes.png', sep = ''), height = 9, width = 16)
...
for(n in 1:length(gards)){
  ...
  for(y in 1:length(years)){
    ...
    # save
    pred.zi_noREs[[count]] <- out1
    pred.zi_REs[[count]] <- out2
    ...
    names(pred.zi_noREs)[[count]] <- name
    names(pred.zi_REs)[[count]] <- name
...
#pdf(file = paste('results/model_prediction/validation_leaveOneGardenOutAndPredictYear_', garden_clim_colname, '_vs_', pheno_colname, '1to1axes.pdf', sep = ''), height = 8, width = 12)
par(mfrow = c(3,4))
...
#png(filename = paste('results/model_prediction/validation_leaveOneGardenOutAndPredictYear_', garden_clim_colname, '_vs_', pheno_colname, '1to1axes.png', sep = ''), height = 16, width = 9.6, units = 'in', res = 300)

#par(mfrow = c(8,4)) # for saving as file
par(mfrow = c(4,5)) # visualize within R
...
for(n in 1:length(gards)){
  ...
  for(y in 1:length(years)){
    ...
    title <- paste(gsub('without_', '', names(mods_test)[n]), ' ', year,  sep = '') 
    # zero-inflated model
    out <-  model_plot_R_mortality_new(modN, 
                                       newdata = newdata, 
                                       re.form = NULL, 
...
wrap_plots(mort_plots, ncol = 4)
#ggsave(filename = paste('results/model_prediction/validation_zero-inflated_leaveOneGardenOutAndPredictYear_', garden_clim_colname, '_vs_', pheno_colname, '1to1axes.png', sep = ''), height = 20, width = 12)
